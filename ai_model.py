...
def find_best_network():
    """Scans networks, measures latency, and recommends the best one."""
    available_networks = scan_wifi_networks()
    if platform.system().lower() == "linux":
        print("Live network scanning is disabled on Linux servers.")
        return None # Just exit the function early
    if not available_networks:
        print("No Wi-Fi networks found.")
        return None
    results = [{'ssid': ssid, 'strength': strength, 'latency': float('inf')} for ssid, strength in available_networks.items()]
